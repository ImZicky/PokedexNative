  ...
  return (
    <Box
      style={{
        ...
        margin: 3,
        flex: 2,
        borderColor: commonService.getColorFromType(props.pokemon.type[0].type.name),
        borderStyle: "solid",
        borderWidth: 5
      }}
    >
      <PokeText
        backgroungColor={commonService.getColorFromType(props.pokemon.type[0].type.name)}
        color={"#ffffff"}
        type="card-id"
      ...
      {
        <Image source={
          props.pokemon.pokeball === "Great Ball" ? require("../../assets/images/pokeballs/greatBall-mini.png") : 
          props.pokemon.pokeball === "Ultra Ball" ? require("../../assets/images/pokeballs/ultraBall-mini.png") : 
          props.pokemon.pokeball === "Master Ball" ? require("../../assets/images/pokeballs/masterBall-mini.png") : 
      ...
      {props.index < 5 &&
        <Image
            ...
            style={{width: 35, height: 35, position: "absolute", right: 43, top: 0}}
          ...
          props.pokemon.shiny ? (
            <View>
              <Image
                ...
                source={{
                  uri: `${commonService.getPokemonMainImageFrontForBattle(props.pokemon.sprites, props.pokemon.shiny)}`,
                }}
              />
              <Image
                style={{width: 25, height: 25, position: "absolute", right: -15, top: 110}}
      ...
      <Flex style={styles.centeredDiv}>
        <PokeText color="#000" type="card-title" text={`${commonService.stringToCapitalLetters(props.pokemon.nickname ? props.pokemon.nickname : props.pokemon.name)}`} />
        <PokeText color="#000" type="card-text-big" text={`Lv${props.pokemon.level}`} />
        <Wrap
          spacing={5}
          style={{
            marginHorizontal: 2,
        ...
        <Wrap mt={10}>
          <Box radius={3} w={151} h={16} style={{backgroundColor: "#4E6648"}}>
            ...
            <Box borderStyle="solid" borderColor={"#4E6648"} border={1} radius={3}
              ...
              h={15}
              style={{backgroundColor: getHpBackgroundColor(props.pokemon.hp, props.pokemon.hpTotal)}}
              />
            </Box>
            </Wrap>

            <Wrap mt={10}>
              <Box radius={5} w={151} h={16} style={{backgroundColor: "#4E6648"}}>
                <View style={{position: "absolute", zIndex: 200, bottom: 0, left: 2}}>
                  <PokeText
                    color={"#000"}
                    text={`XP: ${props.pokemon.levelXp}/${(props.pokemon.level + 1) * 100}`}
                    type={"battle-enemy-card-level"}
                    />
                </View>
              <Box 
            ...
            <View
              style={{backgroundColor: commonService.getColorFromType(props.pokemon.type[0].type.name), width: 155, padding: 10, marginTop: 10}}
            >
              <View style={{marginTop: 10}}>
                ...
                />
                  {
                    props.pokemon.skills && props.pokemon.skills.map((x, i) => (
                      <View style={{marginTop: 7}} key={`pokemon_${props.pokemon.name}_skill_${i}`}>
                        <PokeText
                          color="#FFF"
                          type="card-text"
                          text={`[${x.name}]`}
            ...
            <View style={[styles.modalViewModal, {borderColor: "#4E6648", borderStyle: "solid", borderWidth: 5}]}>
              {props.items.find(x => x.quantity > 0) ? (                      
                props.items.map((item, i) => (
                  item.quantity > 0 &&  item.category === "Heal" && (
